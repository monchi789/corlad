...
export default function EditarPagos() {
  ...
  useEffect(() => {
    const fetchPagoData = async () => {
      if (id) {
        try {
          const res = await getPagoById(parseInt(id));
          ...
          setColegiado(pagoData.id_colegiado);

          // Establecer el método de pago
          setSelectedMetodoPago(pagoData.id_metodo_pago.nombre_metodo_pago.toLowerCase());

          // Cargar tarifas
          const tarifasRes = await getAllTarifas();
          const allTarifas = tarifasRes.data;
          setAllTarifaList(allTarifas);

          // Establecer las tarifas seleccionadas
          const selectedTarifas = allTarifas.filter((tarifa: Tarifa) =>
            pagoData.tarifas.some((pagoTarifa: number) => pagoTarifa === tarifa.id)
          );
          setSelectedTarifaList(selectedTarifas);

          // Verificar si hay mensualidad y establecer meses
          const mensualidadTarifa = selectedTarifas.find((t: { nombre_tarifa: string; }) => t.nombre_tarifa === 'Mensualidad' || t.nombre_tarifa.startsWith('Mensualidad ('));
          if (mensualidadTarifa) {
            setIsMensualidad(true);
            setSelectedMonths(pagoData.meses_pagados || []);
          }
          ...
          <div className="flex flex-row space-x-10">
            <div className="flex flex-col w-full lg:w-2/4">
              ...
              <BuscarColegiadoPagos
                ...
                colegiado={colegiado as Colegiado}
              />
